 ...
 */
JException::JException(const string &txt) :
		text(txt), code(0), source(""), trace(getStackTrace()) {
 ...
 */
JException::JException(const string &txt, const char *file, int line) :
		text(txt), code(0), trace(getStackTrace()) {
 ...
 */
JException::JException(const string &txt, int c) :
		text(txt), code(c), source(""), trace(getStackTrace()) {
}
 ...
 * @param line Line in file where exception raised
 */
JException::JException(const string &txt, int c, const char *file, int line) :
		text(txt), code(c), trace(getStackTrace()) {
	//std::cerr<<__FILE__<<":"<<__LINE__<<std::endl;
	stringstream ss;
	ss << "File: " << file << ",   Line: " << line << ends;
	source = ss.str();
